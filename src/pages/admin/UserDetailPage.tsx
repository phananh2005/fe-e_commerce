...
export function UserDetailModal({ userId, onClose, onRefreshList }: { userId: number; onClose: () => void; onRefreshList?: () => void }) {
  ...
  return (
    <Modal
      open={true}
      onClose={onClose}
      title={user ? `Thông tin người dùng #${user.uuid}` : "Thông tin người dùng"}
      className="max-w-4xl"
    >
      ...
      {user && (
        <div className="space-y-6">
          ...
          <div>
            <div className="text-sm font-medium text-slate-500">Ngày tham gia</div>
            <div className="mt-1 font-medium text-slate-900">{user.createdAt ? formatDateTime(user.createdAt) : "—"}</div>
          </div>
          {user.modifiedAt && (
            <div className="sm:col-span-2">
              <div className="text-sm font-medium text-slate-500">Cập nhật lần cuối</div>
              <div className="mt-1 text-sm text-slate-900">
                {formatDateTime(user.modifiedAt)} {user.modifiedBy && `bởi ${user.modifiedBy}`}
              </div>
            </div>
          )}
          <div className="sm:col-span-2">
            ...
            <div className="mt-4 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={savingRoles || editRoles.length === 0 || !hasRoleChanges}
          ...
          <div className="sm:col-span-2 border-t border-slate-200 pt-4 mt-2">
            <div className="text-xs text-slate-400 space-y-0.5">
              <p>Tạo: {user.createdAt ? formatDateTime(user.createdAt) : "-"} · Bởi: {user.createdBy ?? "-"}</p>
              <p>Cập nhật: {user.modifiedAt ? formatDateTime(user.modifiedAt) : "-"} · Bởi: {user.modifiedBy ?? "-"}</p>
            </div>
          </div>
        </div>
        </div>
